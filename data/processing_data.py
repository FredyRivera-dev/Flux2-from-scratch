            ...
            for batch_idx in range(num_batches):
                ...
                try:
                    ...
                    for i, (emb, prompt_str) in enumerate(zip(emb_batch, prompts)):
                        filename = f"sample_{sample_idx:06d}.safetensors"
                        filepath = self.output_dir / filename
 
                        # Just the tensor in the .safetensors
                        save_file({"emb": emb.cpu().contiguous()}, filepath)
 
                        # Non-tensor metadata in the manifest
            ...
            for batch_idx in range(num_batches):
                ...
                try:
                    ...
                    for latent, img_data in zip(latents_batch, valid_data):
                        filename = f"latent_{latent_idx:06d}.safetensors"
                        filepath = self.output_dir / filename
 
                        save_file({"latents": latent.cpu().contiguous()}, filepath)
